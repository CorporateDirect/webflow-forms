...
def add_conditional_visibility_system_fixed():
    # Read the file
    with open('src/webflow-forms.js', 'r') as f:
        content = f.read()
    
    # Add the conditional visibility system before validateCurrentStep
...
'''
    ...
    # Insert before validateCurrentStep function
    validate_current_step_pos = content.find('        // Validate all fields in current step\n        validateCurrentStep: function(showErrors = true) {')
    if validate_current_step_pos != -1:
        content = content[:validate_current_step_pos] + conditional_visibility_system + content[validate_current_step_pos:]
        print('✅ Added conditional visibility system before validateCurrentStep')
    else:
        print('❌ Could not find validateCurrentStep to insert before')
        return False
...
if __name__ == '__main__':
    if add_conditional_visibility_system_fixed():
        ...
        print('')
        print('🎯 FOR YOUR CALIFORNIA EXAMPLE:')
        print('   <select name="state" id="state">...</select>')
        print('   <input data-hide-if="state:equals:California" name="someField" ... />')
    else:
